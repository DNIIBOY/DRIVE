...
def idm(car: Car, config: SimulationConfig):
    ...
    if car.next:
        ...
        distance_perception_deviation = np.random.normal(0, 1) # En normalfordeling til percieved distance
        s_percieved = max(s + distance_perception_deviation, 0.01)
        
        accel_formular_term_1 = (v / v0)**4
        s_star = s0 + T * v + max((v * delta_v) / (2 * math.sqrt(max(a_max, 1) * max(b, 1))), 0)
        
        
        accel_formular_term_2 = (s_star / s_percieved)**4 # Reducer exponent for at få mindre aggressiv bremse
