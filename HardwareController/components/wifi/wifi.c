...
static void initialize_led(void) {
    ...
    gpio_set_level(LED_PIN, 0);                    // Set LED to OFF initially
}
                                ...
                                int32_t event_id, void* event_data)
{
    if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
        esp_wifi_connect();
        gpio_set_level(LED_PIN, 0); // Turn LED OFF while connecting
        ...
        if (s_retry_num < WIFI_MAXIMUM_RETRY) {
            ...
            gpio_set_level(LED_PIN, 0); // Turn LED OFF on failure
        }
        ...
        gpio_set_level(LED_PIN, 1); // Turn LED ON when connected
    }
}
